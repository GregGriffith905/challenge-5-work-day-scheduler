...
//if (savedItems != null) var items = savedItems;
var saved = [];
...
for (var i = 9; i<=17; i++){  //loop to create html elements for timeblocks from 9am - 5pm 
  ...
  saved[i-9] = localStorage.getItem("timeBlock-"+i); //load saved schedule items
  if (saved[i-9] != null) textArea.text(saved[i-9]);
  if (saved[i-9] == "") saved[i-9]= null;
...
$(function () {
  ...
  $("button").click(function(){
    ...
    saved[(this.id)-9]= buttonParent.children[1].value;
    console.log("new task @ timeblock-" + this.id + ": "  + saved[(this.id)-9]);
    localStorage.setItem(buttonParent.id, saved[(this.id)-9]);
    console.log(saved);
